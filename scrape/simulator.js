...
const decay = n => Math.max(0, Math.min(1, 1 - 0.990*Math.exp(0.020*(n - 450))))
const recursiveDecay = n => n === 0 ? 1 : decay(n)*recursiveDecay(n-1)
...
export function projection(x0, u, interations) {
  let x = x0.clone()
...
function interpolate(n, scale) {
  return Math.max(0, Math.min(1, n/scale))
}

const sale = (n, baseSales, slope, baseEV) =>
  Math.min(0.8*baseSales, n * Math.max(0, slope) + baseEV)

export function basicProjection(uStart, startDate) {
  ...
  const combustionProjection = run.map((x, i) => ({ x: startDate.plus({ months: i }) , y: sum(multiply(sumCombustionMatrix, x)) }))
  const bevProjection =  run.map((x, i) => ({ x: startDate.plus({ months: i }), y: sum(multiply(sumBEVMatrix, x)) }))

  // const combustionSeries = {
  //   label: 'Total-None-Bev',
  //   data:  run.map((x, i) => ({ t: startDate.plus({ months: i }) , y: sum(multiply(sumCombustionMatrix, x)) }))
  // }

  // const bevSeries = {
  //   label: 'Total-BEV',
  //   data:  run.map((x, i) => ({ t: startDate.plus({ months: i }), y: sum(multiply(sumBEVMatrix, x)) }))
  // }
  ...
  //   data: Array(T).fill(0).map((_, i) => ({ t: startDate.plus({ months: i }), y: 12*u(i).get([1, 0]) }))
  // }
  ...
  return {
    noneElectric: combustionProjection,
    electric: bevProjection,
  }
}
