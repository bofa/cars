...
const countries = ['SE', 'DE', 'FI', 'NO', 'FR', 'DK', 'NL', 'CH', 'ES', 'IE', 'GB', 'IT']
...
const trans = m => m[0].map((x, i) => m.map(x => x[i]));
...
(async () => {
  ...
  for (let country of countries) {
    ...
    const jsonString = await fs.readFile('scrape/output/' + country + '.json', 'utf-8').catch(() => "{}");
    const data = JSON.parse(jsonString);

    // console.log('data', data);

    const merge = { ...data, ...resultBrands };

    await fs.writeFile('scrape/output/' + country + '.json', JSON.stringify(merge, null, 2));
  }
